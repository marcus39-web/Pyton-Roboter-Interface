...
1. **Datensammlung:**
   - Im Lernmodus werden alle relevanten Daten (Befehle, Sensorwerte, Zeitstempel) in `learning_data.jsonl` gespeichert (ein JSON pro Zeile).
2. **Training:**
   - Die gesammelten Daten können mit Python-ML-Bibliotheken (z. B. scikit-learn, TensorFlow, PyTorch) für das Training eigener Modelle genutzt werden.
3. **Integration:**
   - Trainierte Modelle können als `.pkl` (scikit-learn), `.h5` (TensorFlow/Keras) oder in anderem Format gespeichert und im Projekt geladen werden.
4. **Inference:**
   - Im Live-Betrieb kann das Modell genutzt werden, um z. B. Sensordaten zu klassifizieren oder Aktionen vorherzusagen.

## Beispiel: Trainingsdaten (learning_data.jsonl)
...
## Beispiel: Modellintegration (scikit-learn)
...
- Die KI-Logik ist aktuell als Platzhalter vorbereitet und kann durch eigene Modelle ersetzt werden.
- Für eigene Experimente: Siehe `learning_data.jsonl` und die zugehörigen Python-Skripte.
- Empfohlene Bibliotheken: `scikit-learn`, `pandas`, `joblib`, `tensorflow`, `torch` (je nach Anwendungsfall)
- Die Integration erfolgt in den Hauptskripten (`main.py`, `categorization_db.py` etc.) an den Stellen, wo Entscheidungen getroffen werden.
  **Fragen zur KI-Integration?** Gerne ein Issue auf GitHub eröffnen oder per Mail melden.
...
- ✅ Lernmodus mit Datenspeicherung in `learning_data.jsonl` (KI-Trainingsdaten)
- ✅ KI-gestützte Entscheidungslogik und Kategorisierung (Vorbereitung für Machine Learning)
...
**Hinweis KI:**
Die Kategorisierung kann durch Machine-Learning-Modelle unterstützt werden. Trainingsdaten werden im Lernmodus automatisch gesammelt und können für eigene Modelle genutzt werden (siehe Abschnitt „Eigene KI-Modelle einbinden“).
...
**Beispiel für einen Eintrag in `learning_data.jsonl`:**
...
Diese Daten können für das Training eigener KI-Modelle (z. B. Klassifikatoren) genutzt werden. Die Integration von Modellen ist vorbereitet und kann flexibel erweitert werden.
...
## Eigene KI-Modelle einbinden

Das Projekt ist vorbereitet, um eigene Machine-Learning-Modelle für Entscheidungsfindung und Kategorisierung zu nutzen.
